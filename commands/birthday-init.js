...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('birthday-init')
        .setDescription('Reschedules all birthdays'),
            ...
            Birthdays.findAll({ attributes: ["user", "date"], raw: true })
                .then(birthdayList => {
                    for (const [bid, birthday] of birthdayList.entries()) {
                        ...
                        birthdayGuild.members.fetch(birthday.user)
                            .then(birthdayUser => {
                                ...
                                const birthdayTrigger = fired => {
                                    ...
                                    if ((now.month() == date.month()) && (now.date() == date.date())) {
                                        ...
                                        birthdayChannel.setName(`happy-birthday-${birthdayUser.user.username}`)
